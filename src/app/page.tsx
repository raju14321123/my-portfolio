    ...
    <div className="fixed top-6 right-6 z-40">
      <div className="bg-black/20 backdrop-blur-2xl border border-white/10 p-4 rounded-3xl shadow-2xl flex flex-col items-center gap-3">
        <div className="flex gap-3">
          ...
          <a href="mailto:your-email@example.com" className="px-6 py-2 rounded-full border border-white/20 bg-white/5 hover:bg-white/20 transition-all font-bold text-white text-xs uppercase tracking-widest">Contact</a>
        </div>
...
function BusCinematic() {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8 py-12 flex items-center justify-start group"
         onMouseEnter={() => videoRef.current?.play()}
         onMouseLeave={() => { videoRef.current?.pause(); videoRef.current!.currentTime = 0; setProgress(0); }}>
      <div className="relative w-[65%] aspect-video rounded-[2rem] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] bg-black/40 backdrop-blur-xl shrink-0">
        <video ref={videoRef} src="/bus.mp4" muted playsInline onTimeUpdate={handleTimeUpdate} className="w-full h-full object-cover" />
      </div>

      <div className="relative w-[45%] h-full flex flex-col justify-center pointer-events-none -ml-[10%]">
